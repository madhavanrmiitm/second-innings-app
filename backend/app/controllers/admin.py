...
from app.payloads import UserRole, UserStatus, TicketStatus
...
async def get_system_users(request):
    ...
    try:
        ...
        if not is_registered or user.role != UserRole.ADMIN:
            return format_response(status_code=403, message="Access denied. Only admins can view system users.")
...
async def delete_user(request, userId, validated_data):
    ...
    try:
        ...
        user, is_registered = auth_service.authenticate_user(id_token)
        if not is_registered or user.role != UserRole.ADMIN:
            return format_response(status_code=403, message="Access denied. Only admins can delete users.")
...
async def get_caregivers_for_review(request):
    ...
    try:
        ...
        user, is_registered = auth_service.authenticate_user(id_token)
        if not is_registered or user.role != UserRole.ADMIN:
            return format_response(status_code=403, message="Access denied. Only admins can view caregivers for review.")
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    (UserRole.CAREGIVER.value, UserStatus.PENDING_APPROVAL.value,)
                )
...
async def review_caregiver(request, caregiverId, validated_data):
    logger.info(f"Executing review_caregiver controller logic for caregiver ID: {caregiverId}.")
    try:
        auth_header = request.headers.get("Authorization")
        if not auth_header or not auth_header.startswith("Bearer "):
            return format_response(status_code=401, message="Authorization header missing or invalid.")
        ...
        user, is_registered = auth_service.authenticate_user(id_token)
        if not is_registered or user.role != UserRole.ADMIN:
            return format_response(status_code=403, message="Access denied. Only admins can review caregivers.")
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    (new_status.value, caregiverId, UserRole.CAREGIVER.value, UserStatus.PENDING_APPROVAL.value,)
                )
                if cur.rowcount == 0:
                    return format_response(status_code=404, message="Caregiver not found or not in pending approval status.")

        return format_response(status_code=200, message=f"Caregiver status updated to {new_status.value} successfully.")
...
async def get_tickets_for_support(request):
    ...
    try:
        ...
        user, is_registered = auth_service.authenticate_user(id_token)
        if not is_registered or user.role not in [UserRole.ADMIN, UserRole.SUPPORT_USER]:
            return format_response(status_code=403, message="Access denied. Only admins or support users can view all tickets.")
...
async def resolve_ticket(request, ticketId, validated_data):
    ...
    try:
        ...
        user, is_registered = auth_service.authenticate_user(id_token)
        if not is_registered or user.role not in [UserRole.ADMIN, UserRole.SUPPORT_USER]:
            return format_response(status_code=403, message="Access denied. Only admins or support users can resolve tickets.")
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                if ticket_status_data[0] == TicketStatus.RESOLVED.value:
                    return format_response(status_code=400, message="Ticket is already resolved.")
...
async def get_ticket_stats(request):
    ...
    try:
        ...
        user, is_registered = auth_service.authenticate_user(id_token)
        if not is_registered or user.role not in [UserRole.ADMIN, UserRole.SUPPORT_USER]:
            return format_response(status_code=403, message="Access denied. Only admins or support users can view ticket statistics.")
