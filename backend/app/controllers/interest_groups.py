...
async def create_interest_group(request, validated_data):
    ...
    try:
        ...
        if not is_registered or user.role != UserRole.INTEREST_GROUP_ADMIN:
            return format_response(
                status_code=403,
                message="Access denied. Only interest group admins can create groups.",
            )
...
async def update_interest_group(request, groupId, validated_data):
    ...
    try:
        ...
        if not is_registered or user.role != UserRole.INTEREST_GROUP_ADMIN:
            return format_response(
                status_code=403,
                message="Access denied. Only interest group admins can update groups.",
            )
...
async def join_interest_group(request, groupId, validated_data):
    logger.info(
        f"Executing join_interest_group controller logic for group ID: {groupId}."
    )
    try:
        auth_header = request.headers.get("Authorization")
        if not auth_header or not auth_header.startswith("Bearer "):
        ...
        if not is_registered or user.role != UserRole.SENIOR_CITIZEN:
            return format_response(
                status_code=403,
                message="Access denied. Only senior citizens can join interest groups.",
            )
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                return format_response(
                    status_code=200,
                    message="Successfully joined interest group (membership not persisted in DB yet).",
                )
...
async def leave_interest_group(request, groupId, validated_data):
    logger.info(
        f"Executing leave_interest_group controller logic for group ID: {groupId}."
    )
    try:
        auth_header = request.headers.get("Authorization")
        if not auth_header or not auth_header.startswith("Bearer "):
        ...
        if not is_registered or user.role != UserRole.SENIOR_CITIZEN:
            return format_response(
                status_code=403,
                message="Access denied. Only senior citizens can leave interest groups.",
            )
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                return format_response(
                    status_code=200,
                    message="Successfully left interest group (membership not persisted in DB yet).",
                )
