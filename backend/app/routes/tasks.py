...
from app.payloads import CreateTask, UpdateTask, CreateReminder, UpdateReminder, TokenRequest
...
async def update_reminder(request: Request, reminderId: int, validated_data: UpdateReminder):
    return await tasks_controller.update_reminder(request, reminderId, validated_data)

@router.post("/reminders/{reminderId}/snooze")
@validate_body(TokenRequest)
async def snooze_reminder(request: Request, reminderId: int, validated_data: TokenRequest):
    return await tasks_controller.snooze_reminder(request, reminderId, validated_data)

@router.delete("/reminders/{reminderId}")
@validate_body(TokenRequest)
async def cancel_reminder(request: Request, reminderId: int, validated_data: TokenRequest):
