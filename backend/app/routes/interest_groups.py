...
from app.payloads import TokenRequest, CreateInterestGroup, UpdateInterestGroup
...
async def create_interest_group(request: Request, validated_data: CreateInterestGroup):
    return await interest_groups_controller.create_interest_group(request, validated_data)
...
async def update_interest_group(request: Request, groupId: int, validated_data: UpdateInterestGroup):
    return await interest_groups_controller.update_interest_group(request, groupId, validated_data)

@router.post("/interest-groups/{groupId}/join")
@validate_body(TokenRequest)
async def join_interest_group(request: Request, groupId: int, validated_data: TokenRequest):
    return await interest_groups_controller.join_interest_group(request, groupId, validated_data)

@router.post("/interest-groups/{groupId}/leave")
@validate_body(TokenRequest)
async def leave_interest_group(request: Request, groupId: int, validated_data: TokenRequest):
    return await interest_groups_controller.leave_interest_group(request, groupId, validated_data)